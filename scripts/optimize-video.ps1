...
if (-not (Test-Path "public/video-intro.mp4")) {
    Write-Host "❌ Vídeo original não encontrado em public/video-intro.mp4" -ForegroundColor Red
    exit 1
}

# Criar backup
Write-Host "💾 Criando backup do vídeo original..." -ForegroundColor Blue
Copy-Item "public/video-intro.mp4" "public/video-intro-backup.mp4"

# Otimizar MP4 (qualidade balanceada)
Write-Host "🔧 Otimizando MP4..." -ForegroundColor Blue
ffmpeg -i "public/video-intro.mp4" `
    -c:v libx264 `
    -crf 28 `
    -preset medium `
    -c:a aac `
    ...
    -movflags +faststart `
    -y `
    "public/video-intro-optimized.mp4"
...
Move-Item "public/video-intro-optimized.mp4" "public/video-intro.mp4" -Force

# Mostrar estatísticas
Write-Host "📊 Estatísticas dos arquivos:" -ForegroundColor Green
Write-Host "MP4 otimizado:" -ForegroundColor Yellow
Get-ChildItem "public/video-intro.mp4" | Select-Object Name, Length, @{Name="Size(MB)";Expression={[math]::Round($_.Length/1MB,2)}}
Write-Host "WebM:" -ForegroundColor Yellow
Get-ChildItem "public/video-intro.webm" | Select-Object Name, Length, @{Name="Size(MB)";Expression={[math]::Round($_.Length/1MB,2)}}
Write-Host "Backup:" -ForegroundColor Yellow
Get-ChildItem "public/video-intro-backup.mp4" | Select-Object Name, Length, @{Name="Size(MB)";Expression={[math]::Round($_.Length/1MB,2)}}
